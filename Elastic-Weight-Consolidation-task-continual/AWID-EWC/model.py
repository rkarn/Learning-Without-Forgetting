...
class Model:
    ...
    def __init__(self, x, y_, task_cl_lab):
        ...
        # layer-1
        self.w1 = weight_variable([self.in_dim, 20])
        self.b1 = bias_variable([20])

        # layer-2
        self.w2 = weight_variable([20, self.out_dim])
    ...
    def compute_fisher(self, imgset, sess, num_samples=200, plot_diffs=False, disp_freq=10):
            ...
            for i in range(num_samples):
                ...
                if sess.run(class_ind, feed_dict={self.x: imgset[im_ind:im_ind+1]}) == 17:
                    ders = sess.run(tf.gradients(tf.log(probs[0,class_ind-1]), self.var_list), feed_dict={self.x: imgset[im_ind:im_ind+1]})
                else:
